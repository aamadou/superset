...
if GITHUB_RELEASE_TAG_NAME:
    output = (
        run_cmd(
            f"source ./scripts/tag_latest_release.sh {GITHUB_RELEASE_TAG_NAME} --dry-run"
        )
        or ""
    )
    match = re.search("SKIP_TAG=(.*)", output)
    skip_tag_value = match.group(1) if match else ""
    if skip_tag_value == "SKIP_TAG::false":
        LATEST_TAG = "latest"
